...
export function ContasScreen() {
  ...
  const handleSave = async () => {
    ...
    // Tratamento do dia de fechamento (entre dia 1 e 31)
    const parsedClosingDay = parseInt(closingDay) || 31
    const finalClosingDay = Math.min(31, Math.max(1, parsedClosingDay))
      ...
      const updated = accounts.map(a =>
        a.id === editAccount.id
          ? {
            ...
            color: selectedColor,
            icon: selectedIcon,
            closingDay: type === 'cartao_credito' ? finalClosingDay : undefined
          }
          : a
      )
            ...
            return (
              <View key={acc.id} style={[styles.accountCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                ...
                <View style={styles.accountRight}>
                  <Text style={[styles.accountBalance, { color: acc.balance < 0 ? colors.destructive : colors.foreground }]}>
                    {formatCurrency(acc.balance)}
                  </Text>
                  <View style={styles.accountActions}>
                    <TouchableOpacity onPress={() => openEdit(acc)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                      <Ionicons name="pencil-outline" size={16} color={colors.mutedForeground} />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => handleDelete(acc)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      <Modal visible={modalVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setModalVisible(false)}>
        <View style={[styles.modal, { backgroundColor: colors.background, paddingTop: insets.top + 16 }]}>
          <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
            ...
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>{editAccount ? 'Editar conta' : 'Nova conta'}</Text>
          ...
          <ScrollView contentContainerStyle={styles.modalContent} showsVerticalScrollIndicator={false}>
            ...
            <TextInput style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} value={name} onChangeText={setName} placeholder="Ex: Nubank, Bradesco..." placeholderTextColor={colors.mutedForeground} />

            <Text style={[styles.fieldLabel, { color: colors.mutedForeground }]}>
              {type === 'cartao_credito' ? 'Limite do Cartão (R$)' : 'Saldo Atual (R$)'}
            </Text>
            <TextInput style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} value={balance} onChangeText={setBalance} placeholder="0,00" placeholderTextColor={colors.mutedForeground} keyboardType="decimal-pad" />

            {/* 👉 NOVO CAMPO: Dia de Fechamento */}
            {type === 'cartao_credito' && (
              <>
                <Text style={[styles.fieldLabel, { color: colors.mutedForeground }]}>Dia de Fechamento da Fatura</Text>
                <TextInput
            ...
            <View style={styles.typeGrid}>
              {ACCOUNT_TYPES.map(t => (
                <TouchableOpacity key={t.value} style={[styles.typeChip, { borderColor: colors.border, backgroundColor: type === t.value ? colors.primary : colors.card }]} onPress={() => setType(t.value)}>
                  <Text style={[styles.typeChipText, { color: type === t.value ? '#FFF' : colors.foreground }]}>{t.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.iconGrid}>
              {ACCOUNT_ICONS.map(item => (
                <TouchableOpacity key={item.icon} style={[styles.iconItem, { backgroundColor: selectedIcon === item.icon ? selectedColor : colors.secondary, borderColor: selectedIcon === item.icon ? selectedColor : 'transparent' }]} onPress={() => setSelectedIcon(item.icon)}>
                  <Ionicons name={item.icon as any} size={20} color={selectedIcon === item.icon ? '#FFF' : colors.mutedForeground} />
                  <Text style={[styles.iconLabel, { color: selectedIcon === item.icon ? '#FFF' : colors.mutedForeground }]}>{item.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  totalValue: { color: '#FFF', fontSize: 34, fontWeight: '700', letterSpacing: -1 },
  ...
  accountsList: { gap: 10 },
  accountCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 16, borderWidth: 1, overflow: 'hidden', paddingRight: 14, gap: 12 },
  accent: { width: 4, alignSelf: 'stretch' },
  iconWrap: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginVertical: 14 },
  ...
  fieldLabel: { fontSize: 12, fontWeight: '500', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 4 },
  ...
  iconItem: { width: 72, alignItems: 'center', paddingVertical: 10, borderRadius: 12, gap: 4, borderWidth: 1.5 },
